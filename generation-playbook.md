...
等待判断阶段展示“双相取点台”：两张自拍图上叠加面部点阵、五官连线、三停参考线、水墨扫描线和候点状态。中央再放置“AI 复核镜”，让甲相和乙相带关键点交替放大显示，配合扫线和轻微缩放动画；即使模型提前完成，也至少停留约 9 秒，完成多轮复核。分析页展示“实测入相”，把测量分值与古法术语并列，方便用户看见“测量 -> 术语 -> 关系象意”的链路。

## 2.2 F-BTI 类型

传播层不只输出古法术语，还要把每个人归入一个温和、可分享的“F-BTI”。类型只表示照片中的面相象意，不判断人格好坏。

当前 8 类：

- 嘴替书生：圆融轮廓、目神回应、表达开。历史参照为苏轼，取舒朗亲和的文士气。
- 慢热高僧：面气柔和、眼神能接话、表达含蓄。历史参照为玄奘，取静定收敛、先听后言之相。
- 排班谋士：眉势有章、脸相有和、表达外放。历史参照为张良，取雅正审时、善定节奏的谋士气。
- 憋招军师：骨线清、眉意深、表达含蓄。历史参照为诸葛亮，取清骨远虑、谋定后动之相。
- 硬控将军：眉峰带劲、行动带风。历史参照为岳飞，取正直劲挺、骨气外显的将帅气。
- 秒懂剑客：目神反应快、表达直接。历史参照为荆轲，取锋利敏捷、临场反应快的剑客气。
- 走神行者：看起来随和，心里有秤。历史参照为徐霞客，取独行观微、边走边校准的行者气。
- 边界镖客：目神有应，边界清醒。历史参照为班超，取守界决断、远行不乱的使者气。
...
快速面相合参只用 F-BTI、本地规则和关系类型，不调用大模型。用户选择回答问诊题后，才把面相摘要、类型关系、问题与选择题回答一并发给合参代理。

关键点取象：

- 三停：额、眉、鼻基、颐部关键点，用于推“形局有序”。
