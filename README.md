...
一个面向抖音小程序方向的关系合参原型。流程为：上传甲乙两张照片，浏览器本地完成面部点阵取象，先生成双相小记和 F-BTI 类型，再根据关系类型、用户卡点和可选问诊回答生成合参建议。
...
- `assets/`：水墨古图与 F-BTI 历史人物头像素材。
- `two-xiangzhi-prd.md`：产品方案与合规边界。
- `generation-playbook.md`：面相取象、F-BTI 和合参生成逻辑手册。
